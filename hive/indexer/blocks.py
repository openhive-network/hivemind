...
class Blocks:
    ...
    def process_blocks(cls, blocks) -> Tuple[int, int]:
        ...
        try:
            ...
            # reptracker sync
            sql_rep = f'SELECT {REPTRACKER_SCHEMA_NAME}.reptracker_block_range_data(:to_block, :from_block);'
            DB.query_no_return(sql_rep, to_block=first_block, from_block=last_num)

            if first_block == last_num:
                log.info(f"Reputation tracker processed block: <{first_block}>")
            else:
                log.info(f"Reputation tracker processed a block range: <{first_block},{last_num}>")
