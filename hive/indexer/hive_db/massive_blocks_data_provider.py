...
blocks_query = """
    SELECT
        ...
        , ht.tx_number as tx_number
        , ho.op_number as op_number
    FROM hive_blocks hb,
    LATERAL ( SELECT COUNT(*) tx_number  FROM hive_transactions ht WHERE ht.block_num = hb.num ) as ht,
    LATERAL ( SELECT COUNT(*) op_number  FROM hive_operations ho WHERE ho.block_num = hb.num AND ( ho.op_type_id < 48 OR ho.op_type_id in (49, 51, 59, 70, 71) ) ) as ho
    WHERE hb.num >= :first AND hb.num < :last
    ORDER by hb.num
    """
