...
def context_detach(db: Db) -> None:
    ...
    if db.is_trx_active():
        # it is required to COMMIT after detach
        # it is impossible to use procedure version of app_context_detach
        # because it is impossible to do this without previously commit transaction with app_nex_block
        db.query_no_return(f"SELECT hive.app_context_detach('{SCHEMA_NAME}')")
        db.query_no_return(f"COMMIT")
    else:
...
def context_attach(db: Db, block_number: int) -> None:
    ...
    if db.is_trx_active():
        # it is required to COMMIT after attach
        # it is impossible to use procedure version of app_context_detach
        # because it is impossible to do this without previously commit transaction with app_nex_block
        db.query_no_return(f"CALL hive.app_context_attach('{SCHEMA_NAME}', {block_number})")
        db.query_no_return(f"COMMIT")
    else:
