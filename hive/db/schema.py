...
def setup(db):
    ...
    sql = """
        ...
        $function$
           ...
           JOIN hive_posts_view hp ON hp.id = trends.id ORDER BY trends.trend DESC LIMIT _limit
        $function$
        language sql STABLE
    """
    db.query_no_return(sql)
    ...
    sql = """
        ...
        CREATE FUNCTION bridge_get_ranked_post_by_created( in _limit SMALLINT )
        RETURNS SETOF bridge_api_post
        AS
        $function$
        SELECT
        ...
        (
            ...
            FROM hive_posts hp1 WHERE hp1.depth = 0 AND NOT hp1.is_grayed
            ORDER BY hp1.created_at DESC
            LIMIT _limit
        ) as created
