...
-- Optimized insert for massive sync: skips account counter updates
DROP FUNCTION IF EXISTS hivemind_app.insert_follows_no_counts;
CREATE OR REPLACE FUNCTION hivemind_app.insert_follows_no_counts(_changes hivemind_app.follow_ids[])
RETURNS INTEGER AS $$
BEGIN
  INSERT INTO hivemind_app.follows (follower, following, block_num)
  SELECT v.follower_id, v.following_id, v.block_num
...
DROP FUNCTION IF EXISTS hivemind_app.delete_follows_no_counts;
CREATE OR REPLACE FUNCTION hivemind_app.delete_follows_no_counts(_changes hivemind_app.follow_ids[])
RETURNS INTEGER AS $$
BEGIN
  DELETE FROM hivemind_app.follows f
  USING UNNEST(_changes) AS v(follower_id, following_id)
...
DROP FUNCTION IF EXISTS hivemind_app.reset_follows_no_counts;
CREATE OR REPLACE FUNCTION hivemind_app.reset_follows_no_counts(_changes hivemind_app.follow_ids[])
RETURNS INTEGER AS $$
BEGIN
  DELETE FROM hivemind_app.follows f
  USING UNNEST(_changes) AS v(follower_id, following_id, block_num)
...
-- Massive sync version of flush_follows that skips account counter updates.
-- After massive sync completes, account counters should be recalculated from
-- the actual follows/muted/etc. tables.
DROP PROCEDURE IF EXISTS hivemind_app.flush_follows_massive CASCADE;
CREATE OR REPLACE PROCEDURE hivemind_app.flush_follows_massive(_follow_updates hivemind_app.follow_updates[], _muted_updates hivemind_app.mute_updates[], _blacklisted_updates hivemind_app.blacklist_updates[], _follow_muted_updates hivemind_app.follow_mute_updates[], _follow_blacklisted_updates hivemind_app.follow_blacklist_updates[], _impacted_accounts TEXT[])
LANGUAGE plpgsql
AS $BODY$
DECLARE
  _count INTEGER;
  ...
  change_follows AS MATERIALIZED (
    SELECT
      CASE upd_with_ids.mode
        WHEN 'insert' THEN (
          hivemind_app.insert_follows_no_counts(upd_with_ids.changes)
        )
        WHEN 'delete' THEN (
          hivemind_app.delete_follows_no_counts(upd_with_ids.changes)
        )
        WHEN 'reset' THEN (
          hivemind_app.reset_follows_no_counts(upd_with_ids.changes)
        )
      END
    FROM (
      SELECT
...
-- Recalculate follower/following counts from actual data.
-- Called after massive sync to fix counts that were skipped during flush_follows_massive.
DROP FUNCTION IF EXISTS hivemind_app.recalculate_follow_counts;
CREATE OR REPLACE FUNCTION hivemind_app.recalculate_follow_counts()
RETURNS VOID AS $$
BEGIN
  -- Recalculate following counts (how many accounts each account follows)
  UPDATE hivemind_app.hive_accounts ha
  SET following = COALESCE(fc.cnt, 0)
  FROM (
    SELECT follower, COUNT(*) AS cnt
    FROM hivemind_app.follows
    GROUP BY follower
  ) AS fc
  WHERE ha.id = fc.follower AND ha.following != fc.cnt;

  -- Recalculate follower counts (how many accounts follow each account)
  UPDATE hivemind_app.hive_accounts ha
  SET followers = COALESCE(fc.cnt, 0)
  FROM (
    SELECT following, COUNT(*) AS cnt
    FROM hivemind_app.follows
    GROUP BY following
  ) AS fc
  WHERE ha.id = fc.following AND ha.followers != fc.cnt;
END
$$ LANGUAGE plpgsql;
