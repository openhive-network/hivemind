...
AS
$BODY$
SELECT hb.created_at FROM hive.blocks hb ORDER BY hb.num DESC LIMIT 1
$BODY$
;
...
AS
$BODY$
SELECT MAX(hb1.num) - CAST( extract(epoch from _time)/3 as INTEGER ) FROM hive.blocks hb1
$BODY$
