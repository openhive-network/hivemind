...
IF EXISTS(SELECT * FROM hivemind_app.hive_db_data_migration WHERE migration = 'hive_mentions fill') THEN
  ...
  PERFORM update_hive_posts_mentions(0, (SELECT hb.num FROM hivemind_app.hive_blocks hb ORDER BY hb.num DESC LIMIT 1) );
...
IF EXISTS (SELECT * FROM hivemind_app.hive_db_data_migration WHERE migration = 'update_hive_post_mentions refill execution') THEN
  ...
  PERFORM update_hive_posts_mentions(0, (select max(num) from hivemind_app.hive_blocks));
