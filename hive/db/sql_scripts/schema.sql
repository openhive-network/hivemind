...
BEGIN
    ...
    (
        ...
        prev       character(40) REFERENCES hivemind_app.hive_blocks (hash) DEFERRABLE,
    ...
    (
        id                     integer GENERATED BY DEFAULT AS IDENTITY                                               NOT NULL PRIMARY KEY,
        root_id                integer                                                                                NOT NULL REFERENCES hivemind_app.hive_posts (id) DEFERRABLE, -- records having initially set 0 will be updated to their id
        parent_id              integer                                                                                NOT NULL REFERENCES hivemind_app.hive_posts (id) DEFERRABLE,
        author_id              integer                                                                                NOT NULL REFERENCES hivemind_app.hive_accounts (id) DEFERRABLE,
    ...
    (
        ...
        post_id      integer                                                                                NOT NULL REFERENCES hivemind_app.hive_posts (id) DEFERRABLE,
        voter_id     integer                                                                                NOT NULL REFERENCES hivemind_app.hive_accounts (id) DEFERRABLE,
        author_id    integer                                                                                NOT NULL REFERENCES hivemind_app.hive_accounts (id) DEFERRABLE,
        permlink_id  integer                                                                                NOT NULL REFERENCES hivemind_app.hive_permlink_data (id) DEFERRABLE,
        ...
        block_num    integer                                                                                NOT NULL REFERENCES hivemind_app.hive_blocks (num) DEFERRABLE,
    ...
    (
        id         integer GENERATED BY DEFAULT AS IDENTITY NOT NULL PRIMARY KEY,
        blogger_id integer                                  NOT NULL REFERENCES hivemind_app.hive_accounts (id) DEFERRABLE,
        post_id    integer                                  NOT NULL REFERENCES hivemind_app.hive_posts (id) DEFERRABLE,
        created_at timestamp without time zone              NOT NULL,
        block_num  integer                                  NOT NULL REFERENCES hivemind_app.hive_blocks (num) DEFERRABLE
    ) INHERITS (hive.hivemind_app);
    ...
    (
        ...
        post_id      integer                                  NOT NULL REFERENCES hivemind_app.hive_posts (id) DEFERRABLE,
        from_account integer                                  NOT NULL REFERENCES hivemind_app.hive_accounts (id) DEFERRABLE,
        to_account   integer                                  NOT NULL REFERENCES hivemind_app.hive_accounts (id) DEFERRABLE,
    ...
    (
        id         integer GENERATED BY DEFAULT AS IDENTITY NOT NULL PRIMARY KEY,
        post_id    integer                                  NOT NULL REFERENCES hivemind_app.hive_posts (id) DEFERRABLE,
        account_id integer                                  NOT NULL REFERENCES hivemind_app.hive_accounts (id) DEFERRABLE,
        block_num  integer                                  NOT NULL
    ) INHERITS (hive.hivemind_app);
    ...
    (
        ...
        dst             integer,                                          -- dst account id except persistent notifs from hivemind_app.hive_notifs
...
BEGIN
    ...
    CREATE INDEX hive_posts_tags_ids_idx ON hivemind_app.hive_posts USING gin (tags_ids gin__int_ops);
...
BEGIN
    ...
    INSERT INTO hivemind_app.hive_state (block_num, db_version) VALUES (0, 0);
    INSERT INTO hivemind_app.hive_blocks (num, hash, created_at, completed)
    VALUES (0, '0000000000000000000000000000000000000000', '2016-03-24 16:04:57', true);
    INSERT INTO hivemind_app.hive_permlink_data (id, permlink) VALUES (0, '');
    INSERT INTO hivemind_app.hive_category_data (id, category) VALUES (0, '');
    INSERT INTO hivemind_app.hive_tag_data (id, tag) VALUES (0, '');
    INSERT INTO hivemind_app.hive_accounts (id, name, created_at, reputation, is_implicit, followers, following, rank,
                                            lastread_at)
    VALUES (0, '', '1970-01-01T00:00:00', DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT);
    INSERT INTO hivemind_app.hive_accounts (name, created_at, reputation, is_implicit, followers, following, rank,
                                            lastread_at)
    VALUES ('miners', '2016-03-24 16:05:00', DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT);
    INSERT INTO hivemind_app.hive_accounts (name, created_at, reputation, is_implicit, followers, following, rank,
                                            lastread_at)
    VALUES ('null', '2016-03-24 16:05:00', DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT);
    INSERT INTO hivemind_app.hive_accounts (name, created_at, reputation, is_implicit, followers, following, rank,
                                            lastread_at)
    VALUES ('temp', '2016-03-24 16:05:00', DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT);
    INSERT INTO hivemind_app.hive_accounts (name, created_at, reputation, is_implicit, followers, following, rank,
                                            lastread_at)
    VALUES ('initminer', '2016-03-24 16:05:00', DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT);
    INSERT INTO hivemind_app.hive_posts(id, root_id, parent_id, author_id, permlink_id, category_id, community_id,
                                        created_at,
...
CREATE OR REPLACE FUNCTION hivemind_app.save_indexes()
    RETURNS VOID
    LANGUAGE 'plpgsql'
AS
$$
...
BEGIN
    RAISE NOTICE 'Attempting to save and drop disableable indexes...';

    INSERT INTO hivemind_app.indexes_constraints(table_name, index_constraint_name, command, is_index, is_foreign_key)
    SELECT tablename,
           indexname,
           indexdef,
           TRUE  as is_index,
    ...
    WHERE schemaname = 'hivemind_app'
      AND indexname = ANY (__indexes)
    ON CONFLICT DO NOTHING;

--     --dropping indexes
--     OPEN __cursor FOR (SELECT ('DROP INDEX IF EXISTS ' || 'hivemind_app' || '.' || index_constraint_name || ';')
--                        FROM hivemind_app.indexes_constraints
--                        WHERE is_index = TRUE);
--
--     LOOP
...
CREATE OR REPLACE FUNCTION hivemind_app.save_and_drop_foreign_keys()
    RETURNS VOID
    LANGUAGE 'plpgsql'
AS
$$
DECLARE
    ...
    __cursor REFCURSOR;
BEGIN
    INSERT INTO hivemind_app.indexes_constraints(index_constraint_name, table_name, command, is_index, is_foreign_key)
    SELECT DISTINCT pgc.conname                          as constraint_name,
                    tc.table_name                        as table_name,
                    'ALTER TABLE ' || 'hivemind_app' || '.' || tc.table_name || ' ADD CONSTRAINT ' || pgc.conname ||
                    ' ' || pg_get_constraintdef(pgc.oid) as command,
                    FALSE                                AS is_index,
                    TRUE                                 as is_foreign_key
    FROM pg_constraint pgc
             JOIN pg_namespace nsp on nsp.oid = pgc.connamespace
             JOIN information_schema.table_constraints tc
                  ON pgc.conname = tc.constraint_name AND nsp.nspname = tc.constraint_schema
    WHERE tc.constraint_type = 'FOREIGN KEY'
      AND tc.table_schema = 'hivemind_app';

    OPEN __cursor FOR (SELECT ('ALTER TABLE ' || 'hivemind_app' || '.' || table_name || ' DROP CONSTRAINT IF EXISTS ' ||
...
CREATE OR REPLACE FUNCTION hivemind_app.restore_foreign_keys()
    RETURNS VOID
    LANGUAGE 'plpgsql'
AS
$$
DECLARE
    ...
    __cursor  REFCURSOR;
BEGIN
    OPEN __cursor FOR (SELECT command
                       FROM hivemind_app.indexes_constraints
                       WHERE is_foreign_key = TRUE);
    LOOP
        FETCH __cursor INTO __command;
