...
BEGIN
  ...
  (
      ...
      WHERE hc.name = _community AND hp1.counter_deleted = 0 AND NOT hp1.is_paidout AND hp1.depth = 0
         ...
         AND (CASE WHEN __observer_id <> 0 THEN NOT EXISTS (SELECT 1 FROM muted_accounts_by_id_view WHERE observer_id = __observer_id AND muted_id = hp1.author_id) ELSE true END)
