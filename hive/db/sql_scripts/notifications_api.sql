...
BEGIN
  ...
  WHERE nv.dst = __account_id  AND hnv.block_num > block_before_head( '90 days' ) AND nv.score >= _min_score AND ( _last_id = 0 OR nv.id < _last_id )
  ORDER BY nv.id DESC
  LIMIT _limit
  ) hnv
...
BEGIN
  ...
  (
    SELECT nv.id, nv.type_id, nv.created_at, nv.src, nv.dst, nv.dst_post_id, nv.score, nv.community, nv.community_title, nv.payload
    FROM hive_raw_notifications_view nv
    WHERE nv.post_id = __post_id AND hnv.block_num > block_before_head( '90 days' ) AND nv.score >= _min_score AND ( _last_id = 0 OR nv.id < _last_id )
    ORDER BY nv.id DESC
    LIMIT _limit
  ) hnv
  JOIN hive_posts hp ON hnv.dst_post_id = hp.id
