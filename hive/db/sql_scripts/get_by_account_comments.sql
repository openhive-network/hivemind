DROP FUNCTION IF EXISTS get_pids_by_account_comments;

CREATE OR REPLACE FUNCTION get_pids_by_account_comments(
  in _author VARCHAR,
...
BEGIN
  ...
  RETURN QUERY SELECT
    ...
    INNER JOIN get_pids_by_account_comments( _author, _permlink, _limit ) as fun
    ON hp.id = fun.id;
END
$function$
