...
BEGIN
  ...
  (
      ...
      WHERE hp1.depth = 0 AND ( ha.reputation >= 1 ) AND ( __post_id = 0 OR hp1.id < __post_id )
...
BEGIN
  ...
  (
      ...
      FROM
          hive_posts hp1
      INNER JOIN hive_accounts ha ON hp1.author_id = ha.id
      WHERE NOT hp1.is_paidout AND ( ha.reputation < 1 ) AND ( hp1.payout + hp1.pending_payout ) > 0
