...
UNION ALL
 SELECT hf.block_num,
    ...
    ''::character varying AS payload
   FROM hivemind_app.hive_follows hf
   JOIN hive.blocks hb ON hb.num = hf.block_num - 1 -- use time of previous block to match head_block_time behavior at given block
...
UNION ALL
 SELECT hm.block_num,
   ...
   JOIN hive.blocks hb ON hb.num = hm.block_num - 1 -- use time of previous block to match head_block_time behavior at given block
) notifs
