DROP FUNCTION IF EXISTS hivemind_postgrest_utilities.raise_exception;
  CREATE OR REPLACE FUNCTION hivemind_postgrest_utilities.raise_exception(_code INT, _message TEXT, _data TEXT = NULL, _id JSON = NULL, _no_data BOOLEAN = FALSE)
RETURNS JSON
LANGUAGE 'plpgsql'
AS
$$
...
DROP FUNCTION IF EXISTS hivemind_postgrest_utilities.raise_operation_param;
CREATE OR REPLACE FUNCTION hivemind_postgrest_utilities.raise_operation_param(_exception_message TEXT,_id JSON)
RETURNS JSON
LANGUAGE 'plpgsql'
AS
$$
...
CREATE OR REPLACE FUNCTION hivemind_postgrest_utilities.raise_uint_exception(_id JSON)
RETURNS JSON
LANGUAGE 'plpgsql'
AS
$$
BEGIN
  RETURN hivemind_postgrest_utilities.raise_exception(-32000, 'Parse Error:Couldn''t parse uint64_t', NULL, _id, TRUE);
END
$$
;
...
CREATE OR REPLACE FUNCTION hivemind_postgrest_utilities.raise_community_exception(_id JSON)
RETURNS JSON
LANGUAGE 'plpgsql'
AS
$$
BEGIN
  RETURN hivemind_postgrest_utilities.raise_exception(-32602,'Invalid parameters','given community name is not valid', _id);
END
$$
;
