...
./$HIVE_NAME sync --pid-file hive_sync.pid --test-max-block=$HIVEMIND_MAX_BLOCK --test-profile=False --steemd-url "$HIVEMIND_SOURCE_HIVED_URL" --prometheus-port 11011 --database-url $DB_URL --mock-block-data-path mock_data/block_data/ --mock-vops-data-path mock_data/vops_data/ --community-start-block 4999998 2>&1 | tee -i hivemind-sync.log
rm hive_sync.pid
